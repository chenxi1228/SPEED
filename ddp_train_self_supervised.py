...
def run(rank, world_size, *, prefix,
    ...
    try:
        ...
        for epoch in range(epoch_start, n_epochs):
            ...
            m_contrast_loss, m_mutual_loss, m_loss, train_epoch_time = \
                train_one_epoch(
            ...
            val_ap, val_auc, ind_val_ap, ind_val_auc, eval_epoch_time = evaluate(
            ...
            val_ap, val_auc = torch.tensor([val_ap]).to(device), torch.tensor([val_auc]).to(device)

            dist.all_reduce(val_ap)
            dist.all_reduce(val_auc)

            val_ap, val_auc = val_ap.item() / world_size, val_auc.item() / world_size

            if not no_ind_val:
                ind_val_ap, ind_val_auc = torch.tensor([ind_val_ap]).to(device), torch.tensor([ind_val_auc]).to(device)

                dist.all_reduce(ind_val_ap)
                dist.all_reduce(ind_val_auc)

                ind_val_ap, ind_val_auc = ind_val_ap.item() / world_size, ind_val_auc.item() / world_size
        ...
        if rank == 0 or 1:
            ...
            if (testing_mode == "from_begin" or testing_mode == "hybrid") and rank == 0:
                model.reset()
                _, _ = eval_edge_prediction(model, test_train_dl, device, restart_mode, uptodate_nodes=uptodate_nodes)
                _, _ = eval_edge_prediction(model, val_dl, device, restart_mode, uptodate_nodes=uptodate_nodes)
            ...
            if testing_mode == "hybrid" and rank == 0:
                ...
                logger.info(f'[Test_from_begin]     ap: {test_ap:.4f}     auc: {test_auc:.4f}')
                logger.info(f'[Test_from_begin] ind ap: {ind_test_ap:.4f} ind auc: {ind_test_auc:.4f}')
